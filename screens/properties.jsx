...
function PropertiesPage({ navigate, favs, toggleFav, mode = 'all' }) {
  ...
  const banner =
    mode === 'rent'
      ? {
          eyebrow: 'For Rent',
          title: 'Residences to lease.',
          kicker:
            'Furnished and unfurnished homes, vetted by Aureva. From month-long stays to multi-year residency.',
        }
      : mode === 'buy'
        ? {
            eyebrow: 'For Sale',
            title: 'Residences to acquire.',
            kicker: 'Trophy homes, pre-construction, and pocket listings across our 38 markets.',
          }
        : {
            eyebrow: 'The Collection',
            title: 'Every residence.',
            kicker: "Browse Aureva's full collection — for sale, for lease, and reserved.",
          };
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--slate)' }}>
              <strong style={{ color: 'var(--navy)', fontWeight: 600 }}>{filtered.length}</strong>{' '}
              residences
            </span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
            ...
            >
              ...
              >
                {['Featured', 'Price ↑', 'Price ↓', 'Area ↓'].map((o) => (
            ...
            <div style={{ display: 'flex', border: '1px solid var(--line-strong)' }}>
              <button
                ...
                style={{
                  padding: 10,
                  color: view === 'grid' ? 'var(--navy)' : 'var(--slate)',
                  background: view === 'grid' ? 'var(--ivory)' : 'transparent',
                }}
              >
                <Icon.Grid />
              </button>
              <button
                ...
                style={{
                  padding: 10,
                  color: view === 'list' ? 'var(--navy)' : 'var(--slate)',
                  background: view === 'list' ? 'var(--ivory)' : 'transparent',
                }}
              >
                <Icon.List />
              </button>
        ...
        {filtersOpen && (
          <div style={{ background: 'var(--ivory)', borderTop: '1px solid var(--line)' }}>
            <div
              className="container filter-grid"
              style={{
                display: 'grid',
        ...
        <div className="container">
          ...
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: 80, color: 'var(--slate)' }}>
              <p className="serif" style={{ fontSize: 28, color: 'var(--navy)' }}>
                No matches.
              </p>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p
          style={{ fontSize: 14, lineHeight: 1.7, color: 'var(--slate)', margin: 0, maxWidth: 500 }}
        >
          {p.description}
        </p>
      ...
      >
        <button
          ...
          style={{ color: faved ? 'var(--gold)' : 'var(--slate)', marginBottom: 14 }}
        >
          <Icon.Heart filled={faved} />
        </button>
        <div className="serif" style={{ fontSize: 28, color: 'var(--gold-deep)', fontWeight: 500 }}>
          {formatPrice(p.price, p.rent)}
        </div>
        <button
          className="btn btn-outline-navy"
          style={{ marginTop: 18, padding: '10px 20px', fontSize: 11 }}
        >
          View details
        </button>
      </div>
