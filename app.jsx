...
const ROUTE_TABLE = [
  ...
  [
    'details',
    /^\/properties\/([^/]+)\/?$/,
    (m) => ({ id: window.PROPERTIES.find((p) => p.slug === m[1])?.id, slug: m[1] }),
  ],
  ['buy', /^\/buy\/?$/, () => ({})],
  ['rent', /^\/rent\/?$/, () => ({})],
  ['sell', /^\/sell\/?$/, () => ({})],
  ...
  [
    'agent',
    /^\/agents\/([^/]+)\/?$/,
    (m) => ({ id: window.AGENTS.find((a) => a.slug === m[1])?.id, slug: m[1] }),
  ],
  ['about', /^\/about\/?$/, () => ({})],
  ['blog', /^\/journal\/?$/, () => ({})],
  [
    'blog-details',
    /^\/journal\/([^/]+)\/?$/,
    (m) => ({ id: window.BLOG_POSTS.find((b) => b.slug === m[1])?.id, slug: m[1] }),
  ],
  ['contact', /^\/contact\/?$/, () => ({})],
  ['privacy', /^\/privacy\/?$/, () => ({})],
  ['terms', /^\/terms\/?$/, () => ({})],
...
function App() {
  ...
  const [favs, setFavs] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('aureva_favs') || '[]');
    } catch (e) {
      return [];
    }
  });
  ...
  const renderPage = () => {
    ...
    switch (route.name) {
      ...
      case 'properties':
        return <PropertiesPage {...common} />;
      case 'buy':
        return <BuyPage {...common} />;
      case 'rent':
        return <RentPage {...common} />;
      case 'sell':
        return <SellPage {...common} />;
      case 'details':
        return <DetailsPage {...common} propertyId={route.params.id || 'p01'} />;
      case 'agents':
        return <AgentsPage {...common} />;
      case 'agent':
        return <AgentProfilePage {...common} agentId={route.params.id || 'a1'} />;
      case 'about':
        return <AboutPage {...common} />;
      case 'blog':
        return <BlogPage {...common} />;
      case 'blog-details':
        return <BlogDetailsPage {...common} postId={route.params.id || 'b1'} />;
      case 'contact':
        return <ContactPage {...common} />;
      case 'privacy':
        return <PrivacyPage {...common} />;
      case 'terms':
        return <TermsPage {...common} />;
      case 'accessibility':
        return <AccessibilityPage {...common} />;
      case 'cookies':
        return <CookiesPage {...common} />;
      default:
        return (
          <div
            className="page"
  ...
  return (
    <div>
      ...
      <main key={route.name + (route.params.id || route.params.slug || '')}>{renderPage()}</main>
